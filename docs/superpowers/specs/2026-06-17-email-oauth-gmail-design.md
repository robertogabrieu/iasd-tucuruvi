...
- `getConfigForSending()` ramifica por `authType`: se `gmail_oauth2`, decifra `client_secret` + `refresh_token`, monta a `ResolvedEmailConfig` OAuth (sender = `email.oauth.senderEmail`); senão, comportamento atual (SMTP). **Se `authType==='gmail_oauth2'` mas não há refresh token (modo escolhido, conta ainda não conectada), lança um `BadRequestError` claro** ("Conta Google não conectada."). O `sendTestEmail` chama `getConfigForSending` **dentro** do try/catch e transforma isso em `{ ok:false, reason }` amigável (hoje a chamada está antes do try — ajustar para dentro). O `/api/contato` já trata falha de envio sem derrubar a resposta.
...
`authorize`/`client`/`disconnect` usam `requireAuth` + `settings:manage` (mutantes também `requireCsrf`). O **callback é a exceção**: validado só pelo `state` (ver §5.3). O `PUT /settings/email` existente passa a aceitar `authType`.
...
- **Gmail (OAuth2)**:
  - O **`from` NÃO é editável** neste modo (o remetente é sempre a conta conectada): em vez do campo, mostrar "Enviando como: `senderEmail`" quando conectado. `to` (destino do teste/contato) segue editável.
...
- **Schemas client em sincronia (convenção do projeto):** adicionar `authType` ao `src/schemas/settings.ts` (`emailSettingsSchema`, usado pelo `useForm` em `Configuracoes.tsx`) — senão o campo é descartado no submit. Os campos do **client OAuth** (`clientId`/`clientSecret`) vão por um **formulário/endpoint separado** (`PUT /settings/email/oauth/client`), com seu próprio schema — não entram no `emailSettingsSchema`.
