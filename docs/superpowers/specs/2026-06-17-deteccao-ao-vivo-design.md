...
3. **Sem live → player mostra o último vídeo** da playlist de cultos (`/api/youtube/cultos?count=1`, Data API já funcionando), incorporado por **ID** (`embed/<videoId>`).
...
- **Domingo:** janela (6) + resto 60min (~23) ≈ **29 → ~2.900 unidades**.
- **Seg–Sex:** **0**.
- Pior dia ≈ **3.900 / 10.000** → folgado. (Se quiser ainda menos, dá pra não checar de madrugada — fora de escopo agora.)

## 5. Backend

### 5.1 Agenda + checagem (`server/lib/youtube.ts` ou novo `server/lib/youtube-live.ts`)
Função pura de agenda + função de status com cache:
- `liveCheckIntervalMs(now)` → decide o TTL pela hora local de SP:
  - Calcular **weekday + HH:MM em `America/Sao_Paulo`** via `Intl.DateTimeFormat('en-US', { timeZone: 'America/Sao_Paulo', weekday, hour, minute, hour12:false })` (ou `formatToParts`). **Não usar `new Date()` do servidor direto** (VPS em UTC).
  - Seg–Sex → `null` (não checa).
  - Sáb/Dom dentro de janela → `10*60_000`; fora → `60*60_000`.
- `getLiveStatus()` (cache em memória `{ value: {isLive,videoId}, expiresAt }`):
  - Se `liveCheckIntervalMs(now)` é `null` (dia útil) → retorna `{ isLive:false, videoId:null }` **sem** chamar a API (e sem cachear chamada).
  - Se cache válido → retorna o cacheado.
  - Senão → chama `searchLiveVideo()` (Data API), guarda com `expiresAt = now + intervalo`. Em erro/sem key → `{ isLive:false }` (loga), **não** derruba a home.
- `searchLiveVideo()` → `GET https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=<CHANNEL_ID>&eventType=live&type=video&maxResults=1&key=<key>`; se houver item, `{ isLive:true, videoId: items[0].id.videoId }`, senão `{ isLive:false, videoId:null }`. `CHANNEL_ID = UCvtcRQ8TcPLZn5dP42bODFg`.
