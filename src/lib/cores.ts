// Espelho de server/core/cores.ts — a fonte da verdade é o arquivo do servidor, e o teste
// (__tests__/eventos/cores.test.ts) roda sobre ele. Mudou lá, copie aqui sem alterar.
// A duplicação existe porque o build do backend tem rootDir em server/ e não alcança src/,
// a mesma razão dos schemas Zod duplicados (CLAUDE.md › Convenções de código).
